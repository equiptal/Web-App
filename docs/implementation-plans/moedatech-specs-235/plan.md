...
- **Risk — backend base URL/env not yet wired** in this repo (only `agentsApiUrl`/`mansourUrl` exist); auth needs `APP_API_URL` + `APP_TENANT_ID` (see Q5). The actual dev/staging URL value is an ops/config input.
...
- 🟡 **Q5 (all real-path ACs) — Backend base URL + tenant env value.** Confirmed: auth routes are `/auth/*` on the **main backend** (`/agents/*` is not in this backend's route list, so it's a distinct base from `AGENTS_API_URL`). Work: add `APP_API_URL` + `APP_TENANT_ID` (default `default`) to `serverEnv`. **Residual (doesn't block tickets):** the actual dev/staging `APP_API_URL` value is an ops/config input to obtain before end-to-end testing/UAT.
