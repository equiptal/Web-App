...
behind; the contents are not.

**Narrowed again later the same day, by the owner, and it is the same sentence carried to its end:**
*"operator docs cannot be viewed or requested and are not part of docs — they are just a view of what the
supplier has."* They are not a quieter kind of document row; **they are outside the document machinery** —
no checkbox in any mode, no place in any batch, no select-all key, and no attention count (AC-75 as
...
| Equipment documents — controls | ⚠️ download only. No view | ✅ **view** per row (V15, AC-69) — **download moved to the batch** (§8.2, owner 2026-08-08) — and **the operator's certificates expose no control at all: no view, no download, and (narrowed later the same day) no checkbox and no ask** (AC-75) |
...
being chosen for the renter.~~ **Withdrawn by the owner, 2026-08-08**, against his own prototype, which
draws **one** «حدّد كل المتاح» and one «إلغاء التحديد (٢)». There is **one link, always**, and at neutral
the **majority of the group's tickable rows** picks it: *"if more than half is available then download; if
more than half is missing, the request will be the enabled one."* The offered link is the one most of the
list would answer, so the common act is one click away. An exact **tie** goes to «حدّد كل المتاح» —
download acts only on the renter's own screen, a request reaches the lessor, and the undecided case should
be the one with no outward consequence. The majority is over **tickable** rows only: a held paper with no
url and every operator certificate (AC-75) are in neither key list, so counting them could hand the
majority to a link that then selected nothing. **The footer is not touched by this** — at neutral both
buttons are still visible and still disabled (AC-77).
...
| ID | Layer | Criterion |
|---|---|---|
| RM3-AC-77 | web | **Given** the equipment documents tab with nothing ticked **Then** every row a batch can answer is tickable and **both** footer buttons render disabled; **When** the renter ticks a **held** row **Then** the selection is in **download** mode, the missing rows dim to 45% and become inert (`disabled`, `aria-disabled`, out of the tab order), «تنزيل» goes live carrying the count and the request button stays disabled; **When** he ticks a **missing** row first **Then** the mirror holds, «اطلب من المؤجّر إرساله» goes live and the held rows go inert; **When** he clears the last tick **Then** the selection returns to **neutral** and every row is tickable again. **And** a **held row with no url** is tickable in **no** mode, and a **held** row is requestable in **none**. **And** an **operator certificate** is tickable in **no** mode and requestable in **none**, absent or held — it is not one of the rows this criterion means by "every row a batch can answer" (AC-75, corrected 2026-08-08: a missing one used to tick for *request*) |
| RM3-AC-78 | web | **Given** a selection in either mode **When** select-all is used **Then** it covers **only that mode's rows** — «حدّد كل المتاح» over the held, «حدّد كل الناقص» over the missing — and **exactly ONE such link renders**, never two; **When** the selection is at **neutral** **Then** the link offered is the one the **majority of the group's tickable rows** would answer — more available than missing ⇒ «حدّد كل المتاح», more missing than available ⇒ «حدّد كل الناقص», **an exact tie ⇒ «حدّد كل المتاح»** — and «إلغاء التحديد (n)» clears back towards neutral; **When** the live button reports a count **Then** it is the count the batch beneath it would actually act on, and the other batch is empty. **And** however a selection was arrived at, a document **request** still names only rows whose status is **missing**. *(Corrected 2026-08-08 against the owner's own prototype, which draws **one** «حدّد كل المتاح» with «إلغاء التحديد (٢)» beside it. The implementation rendered **both** links at neutral, on the reasoning that choosing one would decide the renter's mode for him; the owner overruled it —* "if more than half is available then download; if more than half is missing, the request will be the enabled one." *Two things the ruling left open, decided here: the **tie** falls to «حدّد كل المتاح», because download acts only on the renter's own screen while a request reaches the lessor, so the undecided case should carry no outward consequence; and the majority is counted over the **tickable** rows only — a held paper with no url and every operator certificate (AC-75) are in neither key list, so counting them could hand the majority to a link that then selected nothing. **The footer is unaffected**: at neutral both buttons stay visible and both stay disabled, per AC-77 — this rules on which select-all is offered, not on lighting a button before there is a selection.)* |

| ID | AC | Layer | Where | What it pins |
|---|---|---|---|---|
| RM3-TC-26 | AC-77, AC-78 | web | `tests/unit/machine-panel.test.ts` | **New 2026-08-08 (§8.2).** The mode is inferred from the first tick and is a **pure function of the rows and the ticked set** — no stored mode: a held row ticks *download*, a missing row ticks *request*, nothing ticked is neutral; while a mode holds the other kind is untickable, and clearing the last tick restores all of them; a held row with **no url** and the operator's certificates are tickable in neither mode — **the operator's in neither state, absent or held** (AC-75, corrected 2026-08-08); select-all's two key lists are disjoint, neither can reach the other's rows and **neither contains an operator row**, so the majority that picks the single neutral link is counted over tickable rows alone; each batch is **empty** for the other mode's selection, a row holding several files yields one target per file named apart, and a request draft composed over **every** row still names only the missing ones |
...
| ID | Layer | Criterion |
|---|---|---|
| RM3-AC-73 | web | **Given** any document row in any family **When** the tab renders **Then** a **required** paper renders whether held or not — green and **not requestable** when held, red and counted and requestable when absent — and a **not-required** paper renders only when it is held, carrying no verdict, no colour, no place in the attention count and no checkbox. **Requestable is exactly the missing rows**, and a group with none of them renders no select-all bar. **This criterion does not reach the operator's certificates** — that family is not a document row under it, in either state: it is never requestable, never counted and never tickable (AC-75). *(Amended twice on 2026-08-08. As first written this said only "red and counted and requestable when absent" and left the held case silent, and the implementation read the silence the other way — a required row was selectable held or not, for a legible re-scan. The owner withdrew that: you can only ask for what is not there. Amended again the same day: ~~"any document row in any family"~~ had made a **missing operator certificate** requestable-and-counted by the same clause that governs a missing istimara. The owner withdrew the operator family from the rule entirely — "they are just a view of what the supplier has" — so the scope here is the families that expose files: photos, ownership, equipment certificates.)* |
...
| RM3-AC-75 | web | **Given** the equipment documents tab **When** it renders **Then** the operator's papers are a **third group** with their own rows, each row stating **only whether that certificate is on file** — green or red, **no view, no download, no url** — and the group **participates in nothing else on the tab**: ~~their own attention count~~ **no attention count of its own and no contribution to the tab's badge**, ~~and requestable exactly when it is absent~~ **no checkbox in either mode, no place in either batch and no select-all key, in either state**; covering `operating_license` · `operator_tuv` · `operator_spsp` · `operator_id` · `operator_insurance`, and **not** identified by an `operator_` prefix, which `operating_license` does not carry. The rows are the certificates **this request asked for**, read from `computeUnitReadiness`'s `operatorCerts` (`present`, ignoring `url`); an operator paper nobody asked for raises no row. *(Rewritten twice on 2026-08-08. It read "each viewable, downloadable and requestable". **Withdrawn by the owner**: nothing validates an operator document on upload, so a file the renter can open presents an unchecked upload as verified evidence, and this surface exists to answer* can I trust this? *— presence is a fact the platform can stand behind, the contents are not. The rewrite still left the rows **requestable when absent** and kept the group's **own attention count**, and the implementation shipped a checkbox on every missing certificate that composed into the batch ask. **The owner withdrew that too, in the same breath as the first ruling:* "operator docs cannot be viewed or requested and are not part of docs — they are just a view of what the supplier has." *The count went with it: the pill reads "N rows need action from you", and there is now no action on this group — a number would promise one, and a zero would print «لا ينقص شيء» in green over a red row. The group keeps its heading, its rows and green/red per certificate, and nothing else.)* |
