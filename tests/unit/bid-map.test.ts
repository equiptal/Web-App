...
describe("unitAvailability — the single source of the map's colour (RM3-AC-19)", () => {
  ...
  it("does NOT read inBid — availability is independent of offer membership (owner, 2026-08-13)", () => {
    // The second ruling: *"not confirmed / confirmed available is independent from the in-this-offer
    // chip, it is different."* An `in_offer` orange briefly folded the two questions into one scale,
    // between 975ca79 and its withdrawal. A machine's colour answers "has he named its yard?" and
    // nothing else, so `inBid` must not move it in either direction.
    //
    // The SIGNATURE is the first line of defence — `unitAvailability` takes `Pick<…,"locationSource">`
    // and nothing else, so passing `inBid` is a type error, which is why these calls are cast. The
    // assertions are the second: even handed the flag at runtime, the answer must not move.
    const withFlag = (source: UnitLocationSource, inBid: boolean) =>
      unitAvailability({ ...located(source), inBid } as unknown as Parameters<typeof unitAvailability>[0]);

    for (const inBid of [true, false]) {
      expect(withFlag("unit_yard", inBid)).toBe("confirmed");
      expect(withFlag("listing_yard", inBid)).toBe("unconfirmed");
    }
  });
