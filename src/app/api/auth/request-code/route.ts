...
export async function POST(req: Request) {
  let body: { phone?: string; countryCode?: string; otpMethod?: "SMS" | "WHATSAPP" | "EMAIL"; otpEmail?: string } = {};
  try {
    body = await req.json();
  } catch {
    /* empty body */
  ...
  try {
    const data = await authPost<{ success?: boolean; userId?: number; isNewUser?: boolean; expiresAt?: string }>(
      ...
      // countryCode comes from the phone-field selector (defaults +966); backend accepts E.164.
      { phone, countryCode: body.countryCode ?? "+966", otpMethod: body.otpMethod ?? "SMS", otpEmail: body.otpEmail, role: "rentee" },
