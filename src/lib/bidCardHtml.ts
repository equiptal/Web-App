 ...
 * come from the approved prototype (`email-link-preview.html`): 1px --background border, 10px radius,
...
export function bidCardHtml(card: BidCardPreview, lang: "en" | "ar" = "en"): string {
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="440" dir="${dir}" style="width:440px;max-width:100%;border:1px solid ${COLORS.background};border-radius:${RADII.md};border-collapse:separate;overflow:hidden;background:${COLORS.surface};font-family:'Segoe UI',Roboto,Arial,sans-serif;">
    ...
    <tr><td align="${align}" style="padding:14px 16px 16px;">
      <div style="font-size:13.5px;font-weight:700;color:${COLORS.navy};line-height:1.35;">${escapeHtml(card.title)}</div>
      <div style="font-size:11.5px;color:${COLORS.mutedDark};line-height:1.4;padding-top:7px;">${escapeHtml(card.description)}</div>
      <div style="font-size:10.5px;color:${COLORS.border};letter-spacing:0.4px;padding-top:7px;">${escapeHtml(hostOf(card.url))}</div>
    </td></tr>
  </table>
</a>`;
}
