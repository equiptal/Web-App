...
export function GroupBids({ group }: { group: RequestGroup }) {
  ...
  function downloadQuotation() {
    ...
    // Bilingual: render each supplier's quotation in Arabic and English (current locale first),
    // each as its own page.
    const order = ar ? [true, false] : [false, true];
    const sections = [...bySupplier.values()]
      .map((supBids, si) => order.map((isAr) => renderSection(supBids, si, isAr)).join(""))
      .join("");

    const html = `<!doctype html><html lang="${ar ? "ar" : "en"}" dir="${ar ? "rtl" : "ltr"}"><head><meta charset="utf-8"><title>${esc(quotationFileTitle(group.id, coveredCodes))}</title>` +
